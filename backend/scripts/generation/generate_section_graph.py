#!/usr/bin/env python3
"""
Generate section retrieval graph from master.json
Focuses on sections and subsections for query expansion.
"""
import json
import sys
...
def json_to_mermaid_section(json_data: dict) -> str:
    """Convert section graph JSON to Mermaid format"""
    lines = ["flowchart TD"]
...
def generate_section_graph(document_name: str = "Defence_Standard_00-056_Part_01.txt"):
    """Generate section retrieval graph from master.json"""
    ...
    section_json_path = output_dir / "json" / "section_graph.json"
    section_mermaid_path = output_dir / "mermaid" / "section_graph.mermaid"
    ...
    # Load prompt
    prompt_path = base_dir / "inputs" / "promtps" / "section_graph.txt"
    ...
    try:
        if provider == "anthropic":
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert at generating retrieval-optimized section graphs for semantic search."
                    },
                    {
                        "role": "user",
                        "content": user_message
        ...
        print(f"Section graph has {len(section_data.get('nodes', []))} nodes and {len(section_data.get('edges', []))} edges")

        # Save JSON
        section_json_path.parent.mkdir(parents=True, exist_ok=True)
        with open(section_json_path, "w", encoding="utf-8") as f:
            json.dump(section_data, f, indent=2)
        print(f"Saved section_graph.json to {section_json_path}")

        # Convert to Mermaid
        mermaid_output = json_to_mermaid_section(section_data)

        # Save Mermaid
        section_mermaid_path.parent.mkdir(parents=True, exist_ok=True)
        with open(section_mermaid_path, "w", encoding="utf-8") as f:
            f.write(mermaid_output)
        print(f"Saved section_graph.mermaid to {section_mermaid_path}")

        print("\n✓ Section graph generated successfully!")
        return True
...
if __name__ == "__main__":
    ...
    success = generate_section_graph(document_name)
