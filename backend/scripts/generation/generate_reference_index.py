#!/usr/bin/env python3
"""
Generate reference index graph from master.json
Focuses on external references and their usage across the document.
"""
import json
import sys
...
def json_to_mermaid_reference(json_data: dict) -> str:
    """Convert reference index JSON to Mermaid format"""
    lines = ["flowchart TD"]
    ...
    for node in nodes:
        ...
        # Use different shapes for different kinds
        if kind == "reference":
...
def generate_reference_index(document_name: str = "Defence_Standard_00-056_Part_01.txt"):
    """Generate reference index graph from master.json"""
    ...
    reference_json_path = output_dir / "json" / "reference_index_graph.json"
    reference_mermaid_path = output_dir / "mermaid" / "reference_index_graph.mermaid"
    ...
    # Load prompt
    prompt_path = base_dir / "inputs" / "promtps" / "reference_index_graph.txt"
    ...
    try:
        if provider == "anthropic":
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert at extracting reference dependencies from knowledge graphs."
                    },
                    {
                        "role": "user",
                        "content": user_message
        ...
        reference_data = json.loads(result_text)

        print(f"Reference index has {len(reference_data.get('nodes', []))} nodes and {len(reference_data.get('edges', []))} edges")

        # Save JSON
        reference_json_path.parent.mkdir(parents=True, exist_ok=True)
        with open(reference_json_path, "w", encoding="utf-8") as f:
            json.dump(reference_data, f, indent=2)
        print(f"Saved reference_index_graph.json to {reference_json_path}")

        # Convert to Mermaid
        mermaid_output = json_to_mermaid_reference(reference_data)

        # Save Mermaid
        reference_mermaid_path.parent.mkdir(parents=True, exist_ok=True)
        with open(reference_mermaid_path, "w", encoding="utf-8") as f:
            f.write(mermaid_output)
        print(f"Saved reference_index_graph.mermaid to {reference_mermaid_path}")

        print("\n✓ Reference index graph generated successfully!")
        return True
...
if __name__ == "__main__":
    ...
    success = generate_reference_index(document_name)
